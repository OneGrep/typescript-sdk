 ...
 */
async function fetchAuditLogs(options: { page?: number; pageSize?: number, policyId?: string, action?: string, startDate?: string, endDate?: string }) {
  ...
  logger.info(`Fetching logs with filters: ${JSON.stringify(query_params, null, 2)}`)

  spinner.succeed('Audit logs fetched')
  logger.log(chalk.greenBright.bgBlackBright(JSON.stringify(auditLogs, null, 2)))
...
export const getAuditLogs = new Command()
  ...
  .option('-p, --page <number> (default: 1)', 'Page number to fetch', parseFloat)
  .option('-ps, --page-size <number> (default: 10)', 'Number of items per page', parseFloat)
