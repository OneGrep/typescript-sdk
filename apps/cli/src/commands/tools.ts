...
function validateToolRunOptions(options: {
  integration: string
  tool: string
  getSchema: boolean
  dryRun: boolean
...
async function attemptToolRun(options: {
  ...
  logger.info(`Found ${integrationTools.length} tools for integration ${options.integration}`)
  ...
  if (options.getSchema) {
    logger.log(chalk.bold.blueBright(`Integration: ${tool!.metadata.integrationName}`))
    ...
    logger.log(chalk.greenBright(JSON.stringify({
      "args": schema
    }, null, 2)))
...
export const runTool = new Command()
  ...
  .requiredOption('-i, --integration <string>', 'Integration to run the tool from')
...
export const listIntegrations = new Command()
  ...
  .requiredOption('-i, --integration <string>', 'Integration to list tools from')
  .action(async (options) => {
