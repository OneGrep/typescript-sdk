import { BLTools, blTools, getTool, listFunctions, ListFunctionsResponse, Function as BlaxelFunction } from '@blaxel/sdk'
import { Tool as BlaxelTool } from '@blaxel/sdk/tools/types'
import { McpTool as BlaxelMcpServer, retrieveMCPClient } from '@blaxel/sdk/tools/mcpTool'

import { z } from 'zod';


// // https://github.com/beamlit/sdk-typescript/blob/main/src/tools/types.ts#L3
// export interface BlaxelTool {
//     name: string;
//     description: string;
...
export class BlaxelClient {
    ...
    async refresh(): Promise<boolean> {
        try {
            ...
            console.log(`Discovered servers -> ${JSON.stringify(serverNames, null, 2)}`)
            ...
            for (const serverName of serverNames) {
                ...
                console.log(`Tools for ${serverName} -> ${JSON.stringify(tools.map((t: BlaxelTool) => t.name), null, 2)}`)
            }
