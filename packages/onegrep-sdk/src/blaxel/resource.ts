import { ApiToolResource, BasePolicy, JsonSchema, ToolCallError, ToolCallInput, ToolCallResponse, ToolCustomProperties, ToolId, ToolMetadata, ToolResource } from "types.js";
...
export class BlaxelToolMetadata implements ToolMetadata {
    ...
    constructor(
        ...
        if (apiToolResource.description !== null && apiToolResource.description !== undefined) {
        ...
        if (this.blaxelServerName !== apiToolResource.integration_name) {
            throw new Error(`Blaxel server name ${this.blaxelServerName} does not match integration name ${apiToolResource.integration_name}`)
        }

        if (this.blaxelToolName !== apiToolResource.tool_name) {
            throw new Error(`Blaxel tool name ${this.blaxelToolName} does not match tool name ${apiToolResource.tool_name}`)
        }
        ...
        this.extraProperties = apiToolResource.custom_properties as ToolCustomProperties | undefined
    }
...
export class BlaxelToolResource implements ToolResource {
    ...
    constructor(
        ...
        this.metadata = new BlaxelToolMetadata(blaxelServerName, blaxelTool, apiToolResource)
    ...
    async call(input: ToolCallInput): Promise<ToolCallResponse<z.infer<typeof this._outputZodType>>> {
        try {
